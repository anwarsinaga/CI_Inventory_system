...
class Itemstock extends CI_Controller {
    ...
    function index() {
        ...
        $this->form_validation->set_rules('qty_ld', 'QTY Rusak Ringan', 'trim|xss_clean');
        $this->form_validation->set_rules('qty_hd', 'QTY Rusak Berat', 'trim|xss_clean');
        ...
        if(!isset($_GET['category_qid']) AND !isset($_GET['store_qid'])){
            $item_result = $this->itemstock_model->get();
            $data['itemlist'] = $item_result;
        }
        else{
            $cat_id=$this->input->get('category_qid');
            $st_id = $this->input->get('store_qid');
            $item_result = $this->itemstock_model->getinv($cat_id, $st_id, null);
            $data['itemlist'] = $item_result;
        }
        if(isset($_GET['searchinv'])){
            $searchq = $this->input->get('searchinv');
            $item_result = $this->itemstock_model->getinv(null, null, $searchq);
            $data['itemlist'] = $item_result;
        }
    ...
    public function autosuggest(){
        ...
        $data = $this->itemstock_model->getautosuggest($term); 
        echo json_encode($data);
    }
